...
def weighted_global_l2_loss(pred_force, gt_force, mask, contact_weight=10.0):
    ...
    # 1. Input Cleaning (Good practice, keep this)
    ...
    # 2. Global Squared Difference
    diff = pred_force - gt_force # Error on all vertices
    ...
    # 3. Create Weight Mask
    # Non-contact weight is 1.0, Contact weight is contact_weight.
    weights = torch.ones_like(mask)
    # Set weight to contact_weight where mask is > 0
    ...
    # 5. Normalize by total number of vertices
    # Loss is averaged over all B*T*V vertices.
...
def to_device(device, *args):
    ...
    def _convert(t):
        if isinstance(t, torch.Tensor):
            # If float (float32 or float64), cast to float32
            if t.dtype in [torch.float64, torch.float32]:
                return t.to(device=device, dtype=torch.float32)
            else:
                return t.to(device=device)  # keep other dtypes
        elif isinstance(t, (list, tuple)):
            return type(t)(_convert(x) for x in t)
        else:
            return t  # leave non-tensors as-is
...
def non_negativity_loss_diffusion(pred_force, surface_normals, contact_mask):
    ...
    """
    # Normalize the normals to ensure consistent dot product
    normals_norm = torch.norm(surface_normals, dim=-1, keepdim=True)
    normalized_normals = surface_normals / (normals_norm + 1e-8)
    
    # Calculate the dot product of the force and the outward normal
    ...
    # Apply the contact mask to only consider forces at contact points
    masked_dot_product = (dot_product * contact_mask).clone()
    loss = torch.sum(torch.relu(masked_dot_product)).detach()
    
    # Normalize by the number of contact points
...
def compute_vertex_normals(verts, faces):
    ...
    # Step 2: Accumulate face normals for each vertex
...
def prepare_mano_edges(normalize=False):
    ...
    # Compute normals for the template meshes
...
def translation_loss(t_pred, t_gt):
    ...
    t_pred, t_gt: [B,T,3]
    """
    return F.mse_loss(t_pred, t_gt)  # or F.l1_loss for L1
